...
async function convertPakBdToGeoJSON() {
  try {
    const shapefilePath = join(__dirname, '..', 'Assets', 'Pak Bd', 'pak bd.shp');
    const outputDir = join(__dirname, '..', 'geojson');
    const outputPath = join(outputDir, 'pak-bd.geojson');
    
    // Ensure geojson directory exists
    mkdirSync(outputDir, { recursive: true });
    
    console.log('Converting Pakistan shapefile to GeoJSON...');
